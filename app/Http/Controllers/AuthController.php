...
class AuthController extends Controller
{
    ...
    protected function issueOtp(string $email, OtpEmailSender $mailer, string $purpose)
    {
        ...
        // Safety net: if MAIL_MAILER is still "smtp" (the default) instead of
        // "brevo"/"resend", trying to reach a real SMTP server from Railway
        // hangs (Railway blocks SMTP's ports, and many hosts silently DROP
        // the connection rather than refuse it, so PHP just waits). Without
        // this, that hang runs all the way to PHP's own 30-second execution
        // limit and crashes with a raw, uncatchable "Maximum execution time
        // exceeded" error instead of the friendly message below. Cutting the
        // ceiling to 12 seconds here turns that crash into a normal, fast
        // failure that the try/catch below CAN catch and respond to nicely.
        // (Does nothing when MAIL_MAILER=brevo/resend, which already uses a
        // 20-second HTTPS timeout of its own — see OtpEmailSender.)
        @set_time_limit(12);
